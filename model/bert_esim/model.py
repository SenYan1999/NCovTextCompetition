...
class BertESIM(nn.Module):
    ...
    def __init__(self,
        ...
        self._word_embedding = nn.Embedding(17964,
                                            self.embedding_dim,
        ...
        self.dense = nn.Linear(self.hidden_size + 2*768, num_classes)
    ...
    def forward(self,
        ...
        premises_mask_ = (premises != 0).long().to(self.device)
        hypotheses_mask_ = (hypotheses != 0).long().to(self.device)
        a = self.bert(premises, attention_mask=premises_mask_)[1]
        b = self.bert(hypotheses, attention_mask=hypotheses_mask_)[1]
        ...
        # v_a_avg = torch.max_pool1d(v_ai.permute(0, 2, 1), kernel_size=v_ai.shape[1]).squeeze()
        # v_b_avg = torch.max_pool1d(v_bj.permute(0, 2, 1), kernel_size=v_bj.shape[1]).squeeze()

        v_a_max, _ = replace_masked(v_ai, premises_mask, -1e7).max(dim=1)
        v_b_max, _ = replace_masked(v_bj, hypotheses_mask, -1e7).max(dim=1)

        v = torch.cat([v_a_avg, v_a_max, v_b_avg, v_b_max], dim=1)
        # v = torch.cat([v_a_avg, v_a_max, v_b_avg, v_b_max, a, b], dim=1)
        # v = torch.relu(self.dense(v))

        logits = self._classification(v)
        logits = torch.cat([logits, a, b], dim=-1)
